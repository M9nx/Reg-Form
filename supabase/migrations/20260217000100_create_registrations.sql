...
CREATE UNIQUE INDEX IF NOT EXISTS idx_registrations_full_name_lower ON registrations (lower(full_name));
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE schemaname = 'public'
          AND tablename = 'registrations'
          AND policyname = 'Allow authenticated users to read own registration'
    ) THEN
